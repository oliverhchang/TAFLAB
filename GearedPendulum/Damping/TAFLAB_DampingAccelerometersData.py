...
try:
    ...
    time.sleep(2)
    ser.reset_input_buffer()  # Good practice to clear old data
    ...
    while True:
        try:
            ...
            if not raw_line:
                continue  # Skip if the line is empty (timeout)

            # Now, try to decode the bytes into a string, ignoring errors
            line_of_data = raw_line.decode('utf-8', errors='ignore').strip()

            if line_of_data:
                # Proceed with parsing the clean data
                time_ms, angle_deg = map(float, line_of_data.split(','))
                time_data.append(time_ms / 1000.0)
                angle_data.append(angle_deg)

        except (ValueError, IndexError):
            # This will catch errors if the data is not in "num,num" format
            print(f"Warning: Could not parse line after decoding: '{line_of_data}'")
